function Route(type, parent) {
	this._output = type;
	this._parent = parent;
	this._input = parent ? parent._output : type;
...
function getFieldType(type, field) {
	if (type.prototype && type.prototype.constructor.name == 'model') {
...
function fieldPathType(type, path) {
	var fields = splitFieldPath(path);
	while (type != null && fields.length > 0) {
		var f = fields.shift();
		type = Array.isArray(type) ? type[0] : getFieldType(type, f);
	}
	return type;
}

Route.prototype.field = function(path) {
	var output = fieldPathType(this._input, path);
	var route = this.filter(function(params, cb) {
		var value = params.value;
		var fields = splitFieldPath(path);
