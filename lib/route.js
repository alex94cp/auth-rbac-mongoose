...
Route.prototype.linkWith = function(fieldName) {
	var route = new Route(this._dstType, this);
	route._route = function(err, value, cb) {
		if (err)
			return cb(err);
		...
		return this._dstType instanceof Array
		         ? this._dstType.find(condition, cb)
				 : this._dstType.findOne(condition, cb);
	};
	return route;
};
...
Object.defineProperty(Route.prototype, 'dbRef', {
	get: function() {
		...
		route._route = function(err, value, cb) {
			if (err)
				return cb(err);
			return self._dstType.findById(value, cb);
		};
		return route;
	}
});
